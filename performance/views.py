...
def get_doubt_questions(request):
    ...
    # Create a list to store doubt questions data
    ...
    for question in doubt_questions:
        ...
        doubt_question = {
            ...
            'question': question.question,
            'options': options,
            'explanation': question.explanation,  # Include explanation here
            'subject_name': subject_name,
            'topic_name': topic_name,
            'sub_topic_name': sub_topic_name,
        }
    ...
    if request.method == 'POST':
        ...
        # Fetch explanation for the selected question
    ...
    return render(request, 'performance/doubt_question.html', {
        ...
        'correct_ans': correct_ans,
        'selected_option': selected_option,
        'explanation': explanation,  # Pass explanation to template
    })
...
class PerformanceSummaryView(LoginRequiredMixin,TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return render(request, self.template_name, context)
class SubjectPerformanceView(LoginRequiredMixin, View):
    template_name = 'performance/subject_performance.html'

    def get(self, request, *args, **kwargs):
        performance_data, topics_data = self.get_performance_data(request.user)
        context = {
            'performance_data': performance_data,
            'subjects': Subject.objects.all(),
    ...
    def get_performance_data(self, user):
        ...
        for subject in subjects:
            total_questions_per_topic = self.get_total_questions_per_topic(subject)
            correct_answers_per_topic = self.get_correct_answers_per_topic(user, subject)

            subject_performance = {}
            subject_topics = []
            for topic in total_questions_per_topic:
                topic_name = topic['sub_topic_name__topic_name__topic_name']
                total_questions = topic['total']
                correct_answers = next(
                    (item['correct'] for item in correct_answers_per_topic if item['sub_topic_name__topic_name__topic_name'] == topic_name),
                    0
                )
                performance_percentage = self.calculate_performance_percentage(correct_answers, total_questions)
                subject_performance[topic_name] = {'percentage': performance_percentage}
                subject_topics.append(topic_name)  # Add topic names instead of IDs

            performance_data[subject.subject_name] = subject_performance
            topics_data[subject.subject_name] = subject_topics

        return performance_data, topics_data

    def get_total_questions_per_topic(self, subject):
        questions = Question.objects.filter(sub_topic_name__topic_name__subject_name=subject).select_related('sub_topic_name__topic_name')
        total_questions_per_topic = questions.values('sub_topic_name__topic_name__topic_name').annotate(total=Count('id'))
        return total_questions_per_topic

    def get_correct_answers_per_topic(self, user, subject):
        try:
            user_perf = user_performance.objects.get(user=user)
            answered_correct_ids = [int(id) for id in user_perf.answered_correct.split(';') if id.isdigit()]
            valid_answered_correct_ids = Question.objects.filter(id__in=answered_correct_ids).values_list('id', flat=True)
        ...
        questions = Question.objects.filter(id__in=answered_correct_ids, sub_topic_name__topic_name__subject_name=subject).select_related('sub_topic_name__topic_name')
        correct_answers_per_topic = questions.values('sub_topic_name__topic_name__topic_name').annotate(correct=Count('id'))
        return correct_answers_per_topic

    def calculate_performance_percentage(self, correct_answers, total_questions):
        return round((correct_answers / total_questions) * 100) if total_questions else 0
