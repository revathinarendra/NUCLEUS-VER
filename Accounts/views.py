    ...
    def get(self, request):
        ...
        if user_profiles.exists():
            if user_profiles.count() > 1:
                # Log a warning or handle the duplicate profiles appropriately
                # For example, you might want to notify an admin or clean up duplicates
                #log.warning(f"Multiple UserProfiles found for user {request.user}")
                userprofile = user_profiles.first()  # Just pick the first one
            else:
                userprofile = user_profiles.first()
        else:
