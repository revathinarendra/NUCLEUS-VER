...
#             print("attempted questions:",user_history.attempted_ques)
...
class MCQQuizView(LoginRequiredMixin, View):
    ...
    def post(self, request, topic_slug, question_num):
        ...
        # Ensure that attempted questions are updated
        if question_id not in user_history.attempted_ques.split(';'):
            user_history.attempted_ques += question_id + ';'
            if selected_option == correct_ans:
                user_history.answered_correct += question_id + ';'
            user_history.save()
...
def update_question_history(request):
    ...
    if request.method == 'POST':
        ...
        if not question_id or not action:
            return JsonResponse({'status': 'error', 'message': 'Question ID or action missing'})
        ...
        try:
            ...
            if user_history.bookmark_ques is None:
                user_history.bookmark_ques = ''
            if user_history.revise_ques is None:
                user_history.revise_ques = ''
            ...
            print("important_ids:",important_ids)
